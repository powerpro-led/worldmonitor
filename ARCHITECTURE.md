...
Detected by hostname (the `tech.`/`finance.`/`commodity.`/`happy.`/`energy.` subdomain of the configured `APP_DOMAIN` → that variant) or localStorage. Controls: default panels, map layers, refresh intervals, theme, UI text. Variant change resets all settings to defaults.
...
| File | Purpose |
|------|---------|
| `_cors.js` | Origin allowlist (configured `APP_DOMAIN` + subdomains, Vercel previews, tauri://localhost, localhost) |
