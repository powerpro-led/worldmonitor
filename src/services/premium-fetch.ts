 ...
 *
 * Injects a Supabase Bearer token (or WORLDMONITOR_API_KEY as fallback)
 * directly into every request for premium endpoints. This is the
 * source-of-truth auth injection for those routes — no reliance on the
 * global fetch patch.
 *
 ...
 *      (gateway.ts: `if (isTierGated) resolveSupabaseSession(...)`); for
 ...
 * browser globals (localStorage, Supabase session). Null in production.
 */
let _testProviders: {
  getTesterKey?: () => string;
  getTesterKeys?: () => string[];
  getAuthToken?: () => Promise<string | null>;
  isAuthUserSignedIn?: () => boolean | Promise<boolean>;
} | null = null;
 ...
 *
 * Only the exact shared allowlist opts into Supabase bearer injection. Other
...
const AUTH_TOKEN_RETRY_DELAY_MS = 300;

async function resolveAuthToken(): Promise<string | null> {
  if (_testProviders) {
    return _testProviders.getAuthToken ? _testProviders.getAuthToken() : null;
  }
 ...
 */
async function isAuthUserSignedIn(): Promise<boolean> {
  if (_testProviders) {
    return (await _testProviders.isAuthUserSignedIn?.()) ?? false;
  }
  try {
...
function delayBeforeAuthRetry(): Promise<void> {
  // Test providers stand in for the real auth session, so never block the
  // suite on a real timer.
  const ms = _testProviders ? 0 : AUTH_TOKEN_RETRY_DELAY_MS;
...
export async function premiumFetch(
  ...
  if (isPremiumRpcTarget(input, forcePremium)) {
    try {
      ...
      // Boot-window auth race: while Supabase bootstraps the session, the
      // auth provider's token-cache invalidation bumps the token generation
      // (so a rotating user's stale JWT is never painted), which makes any
      // in-flight getAuthToken() abandon to null. A signed-in user's premium
      // fetch that lands in that window would otherwise fall through to an
      ...
      // present auth user so anonymous visitors fall through immediately.
      if (!token && await isAuthUserSignedIn()) {
        await delayBeforeAuthRetry();
  ...
  // (no API key, no tester key, no Supabase Bearer) the gateway will return
