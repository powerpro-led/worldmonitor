...
export const PREMIUM_RPC_PATHS = new Set<string>([
  ...
  // #3734: auth-gated mutation that enqueues a simulation task. Companion
  ...
  // Path-gated here so premiumFetch attaches the session Bearer for signed-in
  // callers; the server gate in api/mcp-proxy.ts uses isCallerPremium which
  // validates an enterprise key or a session Bearer.
  '/api/mcp-proxy',
  // /api/chat-analyst: auth-gated streaming SSE endpoint for WM Analyst panel.
  ...
  // attaches the Bearer for signed-in callers → every send returned 403
  // "Sign-in or API key required" despite a valid session. Symptom stayed
  // hidden until PR #3797 fixed the unlock-wipe so users could actually type
  // and click Send.
  '/api/chat-analyst',
]);
