...
export class FSIPanel extends Panel {
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      ...
      if (fsiValue <= 0) {
        if (!this._hasData) this.showNotSynced(t('components.fsi.errors.unavailable'), () => void this.fetchData());
        return false;
      }
      ...
      if (!this._hasData) this.showNotSynced(e instanceof Error ? e.message : t('components.fsi.errors.failedToLoad'), () => void this.fetchData());
      return false;
    }
  }
