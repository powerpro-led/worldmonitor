...
test('uses canonical app origin when proxying to cloud fallback (cloudFallback enabled)', async () => {
  ...
  try {
    ...
    const body = await response.json();
    assert.equal(body.source, 'remote');
    assert.equal(body.origin, `https://${TEST_APP_DOMAIN}`);
    assert.equal(remote.origins[0], `https://${TEST_APP_DOMAIN}`);
  } finally {
    await app.close();
    await localApi.cleanup();
    await remote.close();
...
// ── Phase 2: loopback control plane (/api/local-config + settings.html shim) ──

async function withTempConfigDb(fn) {
  const prev = process.env.LOCAL_CONFIG_DB_PATH;
  const prevSkip = process.env.WM_LOCAL_SKIP_RESTART;
  const dbPath = path.join(await mkdtemp(path.join(os.tmpdir(), 'wm-cfgdb-')), 'config.db');
  process.env.LOCAL_CONFIG_DB_PATH = dbPath;
  process.env.WM_LOCAL_SKIP_RESTART = '1';
  try {
    ...
    if (prevSkip === undefined) delete process.env.WM_LOCAL_SKIP_RESTART;
    else process.env.WM_LOCAL_SKIP_RESTART = prevSkip;
    await rm(path.dirname(dbPath), { recursive: true, force: true });
  }
}

test('GET /api/local-config requires the transport token', async () => {
  const localApi = await setupApiDir({});
  const app = await createLocalApiServer({
    port: 0, apiDir: localApi.apiDir, logger: { log() {}, warn() {}, error() {} },
  });
  const { port } = await app.start();
  try {
    const resp = await fetch(`http://127.0.0.1:${port}/api/local-config`);
    assert.equal(resp.status, 401);
  } finally {
    await app.close();
    await localApi.cleanup();
...
test('GET /api/local-config reports every key and never echoes a secret value', async () => {
  await withTempConfigDb(async () => {
    const { setConfig } = await import('./config-store.mjs');
    setConfig('VITE_SUPABASE_URL', 'https://proj.supabase.co');
    setConfig('OPENROUTER_API_KEY', 'sk-or-v1-abcdefabcdefabcdef');
    ...
    const { port } = await app.start();
    try {
      const resp = await authFetch(`http://127.0.0.1:${port}/api/local-config`);
      assert.equal(resp.status, 200);
      const body = await resp.json();
      for (const key of [
        'VITE_SUPABASE_URL', 'VITE_SUPABASE_PUBLISHABLE_KEY', 'UPSTASH_REDIS_REST_URL',
        'UPSTASH_REDIS_REST_READONLY_TOKEN', 'OPENROUTER_API_KEY',
      ]) {
        assert.ok(body[key], `entry for ${key}`);
      }
      assert.equal(body.VITE_SUPABASE_URL.value, 'https://proj.supabase.co');
      assert.equal(body.OPENROUTER_API_KEY.stored, 'set');
      assert.equal(body.OPENROUTER_API_KEY.value, undefined, 'secret value is not echoed');
    } finally {
      await app.close();
      await localApi.cleanup();
    }
...
test('POST /api/local-config validates keys and shapes, then persists + populates process.env', async () => {
  await withTempConfigDb(async (dbPath) => {
    const orIsSet = process.env.OPENROUTER_API_KEY;
    delete process.env.OPENROUTER_API_KEY;
    const localApi = await setupApiDir({});
    const app = await createLocalApiServer({
      port: 0, apiDir: localApi.apiDir, logger: { log() {}, warn() {}, error() {} },
    ...
    const { port } = await app.start();
    try {
      const base = `http://127.0.0.1:${port}/api/local-config`;
      const j = (payload) => authFetch(base, {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload),
      });

      assert.equal((await j({ NOPE: 'x' })).status, 400);
      assert.equal((await j({ VITE_SUPABASE_URL: 'not-a-url' })).status, 400);
      assert.equal((await j({ VITE_SUPABASE_URL: 'http://insecure.example' })).status, 400);

      const ok = await j({ OPENROUTER_API_KEY: 'sk-or-v1-abcdefabcdefabcdef' });
      assert.equal(ok.status, 200);
      const okBody = await ok.json();
      assert.deepEqual(okBody.applied, ['OPENROUTER_API_KEY']);
      assert.equal(okBody.restarting, false, 'WM_LOCAL_SKIP_RESTART suppresses the restart');
      assert.equal(process.env.OPENROUTER_API_KEY, 'sk-or-v1-abcdefabcdefabcdef');

      const { readAllConfig } = await import('./config-store.mjs');
      assert.equal(readAllConfig()[ 'OPENROUTER_API_KEY' ], 'sk-or-v1-abcdefabcdefabcdef');
      assert.ok(dbPath);
    } finally {
      delete process.env.OPENROUTER_API_KEY;
      if (orIsSet !== undefined) process.env.OPENROUTER_API_KEY = orIsSet;
      await app.close();
      await localApi.cleanup();
    }
  });
...
test('DELETE /api/local-config clears a stored key and its process.env value', async () => {
  await withTempConfigDb(async () => {
    const { setConfig } = await import('./config-store.mjs');
    setConfig('OPENROUTER_API_KEY', 'sk-or-v1-abcdefabcdefabcdef');
    const orIsSet = process.env.OPENROUTER_API_KEY;
    process.env.OPENROUTER_API_KEY = 'sk-or-v1-abcdefabcdefabcdef';
    ...
    try {
      const resp = await authFetch(
        `http://127.0.0.1:${port}/api/local-config?key=OPENROUTER_API_KEY`,
        { method: 'DELETE' },
      );
      assert.equal(resp.status, 200);
      const { readAllConfig } = await import('./config-store.mjs');
      assert.equal(readAllConfig()['OPENROUTER_API_KEY'], undefined);
...
test('POST /api/local-login refuses until Supabase is configured', async () => {
  const snapshot = {
    url: process.env.VITE_SUPABASE_URL,
    sup: process.env.SUPABASE_URL,
  ...
  const { port } = await app.start();
  try {
    const resp = await authFetch(`http://127.0.0.1:${port}/api/local-login`, { method: 'POST' });
    assert.equal(resp.status, 400);
  } finally {
    for (const [k, v] of Object.entries({
      VITE_SUPABASE_URL: snapshot.url, SUPABASE_URL: snapshot.sup, VITE_SUPABASE_PUBLISHABLE_KEY: snapshot.key,
    })) {
      if (v === undefined) delete process.env[k];
      else process.env[k] = v;
    }
...
test('settings.html gets the control-panel token shim; dashboard.html (no embed) does not', async () => {
  const snapshot = { url: process.env.VITE_SUPABASE_URL, sup: process.env.SUPABASE_URL };
  process.env.VITE_SUPABASE_URL = 'https://proj.supabase.co';
  const localApi = await setupApiDir({});
  const staticDir = await mkdtemp(path.join(os.tmpdir(), 'wm-static-cp-'));
  await writeFile(path.join(staticDir, 'settings.html'), '<!doctype html><head></head><body>settings</body>', 'utf8');
  await writeFile(path.join(staticDir, 'dashboard.html'), '<!doctype html><head></head><body>dash</body>', 'utf8');

  const app = await createLocalApiServer({
    port: 0, apiDir: localApi.apiDir, staticDir, logger: { log() {}, warn() {}, error() {} },
  });
  const { port } = await app.start();
  try {
    const settings = await (await fetch(`http://127.0.0.1:${port}/settings.html`)).text();
    assert.match(settings, /__WM_LOCAL_CONTROL_PANEL/);
    assert.match(settings, /x-worldmonitor-local-token/);

    const dash = await (await fetch(`http://127.0.0.1:${port}/dashboard.html`)).text();
    assert.doesNotMatch(dash, /__WM_LOCAL_CONTROL_PANEL/);
    assert.doesNotMatch(dash, /x-worldmonitor-local-token/);
  } finally {
    if (snapshot.url === undefined) delete process.env.VITE_SUPABASE_URL;
    else process.env.VITE_SUPABASE_URL = snapshot.url;
    if (snapshot.sup === undefined) delete process.env.SUPABASE_URL;
    else process.env.SUPABASE_URL = snapshot.sup;
    await app.close();
    await localApi.cleanup();
    await rm(staticDir, { recursive: true, force: true });
  }
...
test('first-run redirect is injected into dashboard.html only when Supabase is unconfigured', async () => {
  const snapshot = { url: process.env.VITE_SUPABASE_URL, sup: process.env.SUPABASE_URL };
  delete process.env.VITE_SUPABASE_URL;
  delete process.env.SUPABASE_URL;
  const localApi = await setupApiDir({});
  const staticDir = await mkdtemp(path.join(os.tmpdir(), 'wm-static-fr-'));
  await writeFile(path.join(staticDir, 'dashboard.html'), '<!doctype html><head></head><body>dash</body>', 'utf8');
  await writeFile(path.join(staticDir, 'settings.html'), '<!doctype html><head></head><body>settings</body>', 'utf8');
  ...
  const { port } = await app.start();
  try {
    const dashUnconfigured = await (await fetch(`http://127.0.0.1:${port}/dashboard.html`)).text();
    assert.match(dashUnconfigured, /settings\.html\?firstrun=1/);
    // settings.html itself must never redirect — it's how you fix the config.
    const settings = await (await fetch(`http://127.0.0.1:${port}/settings.html`)).text();
    assert.doesNotMatch(settings, /firstrun=1/);

    process.env.VITE_SUPABASE_URL = 'https://proj.supabase.co';
    const dashConfigured = await (await fetch(`http://127.0.0.1:${port}/dashboard.html`)).text();
    assert.doesNotMatch(dashConfigured, /firstrun=1/);
  } finally {
    if (snapshot.url === undefined) delete process.env.VITE_SUPABASE_URL;
    else process.env.VITE_SUPABASE_URL = snapshot.url;
    if (snapshot.sup === undefined) delete process.env.SUPABASE_URL;
