...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  headers.set('Origin', resolveAppOrigin(process.env.APP_DOMAIN));
...
function resolveConfig(options = {}) {
  ...
  const remoteBase = String(
    options.remoteBase ?? process.env.LOCAL_API_REMOTE_BASE ?? resolveApiOrigin(process.env.APP_DOMAIN),
  ).replace(/\/$/, '');
...
function buildSidecarAllowedOrigins(rawDomain) {
  const base = [
    /^tauri:\/\/localhost$/,
    /^https?:\/\/localhost(:\d+)?$/,
    /^https?:\/\/127\.0\.0\.1(:\d+)?$/,
  ...
  if (isLocalDomain(rawDomain)) return base;
  const domain = normalizeDomain(rawDomain);
  // Only allow exact domain or single-level subdomains (e.g. preview-xyz.<domain>).
  // A (.*\.)? pattern would be overly broad — anchored to a restricted
  // subdomain-label character class to prevent spoofing via domains like
  // <domain>EVIL.vercel.app. escape() is redundant for a normalized bare
  // host (already validated hostname syntax, no regex metacharacters other
  // than the literal `.` this pattern already escapes explicitly).
  const escaped = domain.replace(/\./g, '\\.');
  return [...base, new RegExp(`^https:\\/\\/([a-z0-9-]+\\.)?${escaped}$`)];
}

const SIDECAR_ALLOWED_ORIGINS = buildSidecarAllowedOrigins(process.env.APP_DOMAIN);
...
export async function createLocalApiServer(options = {}) {
  ...
  return {
    ...
    async start() {
      ...
      // (The regional-news digest is no longer warm-pinged here. Under the
      // platform pivot — PLATFORM_ARCHITECTURE.md P2 / Workstream 7 — the
