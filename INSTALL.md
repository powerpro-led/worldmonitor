# Installing WorldMonitor Local (macOS)
...
5. Sign in so your personalised **Latest Brief** loads:
...
6. In VS Code, run **WorldMonitor: Open Local Dashboard** from the command
   palette.

## Managing the backend

```sh
node scripts/worldmonitor-local.mjs status      # backend / launchd / token / identity
...
node scripts/worldmonitor-local.mjs uninstall   # remove the launchd service (keeps ~/.worldmonitor/)
```
...
2. `cd` into it and run `./install.sh` — it rewrites the launchd plist to point
   at the new folder and picks up your existing `~/.worldmonitor/` token and
   session. Copy your old `.env` over if you don't want to re-enter values.
