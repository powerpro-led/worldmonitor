...
describe('embed public data auth', () => {
  ...
  it('lets anonymous iframe requests reach embed RPC handlers while nearby RPCs stay gated', async () => {
    ...
    for (const path of EMBED_PUBLIC_RPC_PATHS) {
      const res = await gateway(new Request(`https://example.test${path}`, {
        headers: { Origin: 'https://example.test' },
      }));
      assert.equal(res.status, 200, `${path} should not require wm-session in an embed iframe`);
    }

    const gated = await gateway(new Request('https://example.test/api/conflict/v1/list-ucdp-events', {
      headers: { Origin: 'https://example.test' },
    }));
  ...
  it('scopes anonymous bootstrap access to the weather embed key only', async () => {
    const publicReq = new Request('https://example.test/api/bootstrap?keys=weatherAlerts', {
      headers: { Origin: 'https://example.test' },
    });
    ...
    const rejected = [
      ...
      'https://example.test/api/bootstrap?tier=fast',
      'https://example.test/api/bootstrap?keys=weatherAlerts,marketQuotes',
      'https://example.test/api/bootstrap?keys=marketQuotes',
      'https://example.test/api/bootstrap?keys=weatherAlerts&debug=1',
      'https://example.test/api/bootstrap?keys=weatherAlerts&keys=marketQuotes',
    ];

    for (const url of rejected) {
      const req = new Request(url, { headers: { Origin: 'https://example.test' } });
